...
class DFS(IAlgorithmn):
    ...
    def percorre_matriz(self, graph):
        if (graph.data is None) or (self.existInList(graph)): return
        self.path.append(graph)
        for vizinho in graph.vizinhos:
            self.percorre_matriz(vizinho)
        
    def existInList(self, node):
